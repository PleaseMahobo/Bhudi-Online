...
export default function RMMDashboard() {
  ...
  useEffect(() => {
    ...
    // Live metrics simulation (replace with real backend later)
      ...
      <div className="w-72 bg-[#0f172a] border-r border-[#1e3a8a] p-6 fixed h-screen overflow-auto">
        ...
        <nav className="space-y-1">
          {['Dashboard', 'Devices', 'Live Alerts', 'Command Center', 'Analytics', 'Settings'].map((item, i) => (
            <a key={i} href="#" className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all ${i === 0 ? 'bg-gradient-to-r from-sky-500 to-blue-600 text-white shadow-lg' : 'hover:bg-zinc-800 text-zinc-300'}`}>
              {item}
            </a>
          ))}
        </nav>
