...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const ok = await auth.login(email, password, needMfa ? mfaCode : undefined);
      if (!ok) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {needMfa && (
            <div className="space-y-2">
              <p className="rounded-xl border border-indigo-500/30 bg-indigo-950/40 px-3 py-2 text-center text-xs text-indigo-200">MFA is already enabled. Enter the current 6-digit code from your authenticator app. No new QR code is required.</p>
              <label className="text-xs font-medium text-slate-400">Authenticator code</label>
              <input value={mfaCode} onChange={(e) => setMfaCode(e.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" maxLength={6} autoFocus className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-center font-mono text-lg tracking-widest text-white outline-none focus:border-indigo-500" placeholder="000000" required />
            </div>
          )}
