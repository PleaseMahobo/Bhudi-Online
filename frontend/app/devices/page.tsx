...
import { listDevicesDetailed, healthGradeClass, type Device } from '@/lib/devices';
...
function HealthBadge({ score, grade }: { score?: number | null; grade?: string | null }) {
  const g = (grade || '').toUpperCase() || null;
  if (score == null && !g) {
  ...
  return (
    <span
      className={
        ...
        healthGradeClass(label)
      }
      title={score != null ? `Health score ${score}/100` : 'Health grade'}
    >
      <span>{label}</span>
                ...
                filtered.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <HealthBadge score={d.health_score} grade={d.health_grade} />
                      </td>
          ...
          >
            ...
            <div className="space-y-5 p-5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <HealthBadge score={selected.health_score} grade={selected.health_grade} />
              </div>
              ...
              <section>
                ...
                <dl className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                    <dt className="text-[11px] text-slate-500">CPU</dt>
                    <dd className="mt-0.5 font-medium text-slate-900">{metric(selected.cpu_percent)}</dd>
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                    <dt className="text-[11px] text-slate-500">Memory</dt>
                    <dd className="mt-0.5 font-medium text-slate-900">{metric(selected.memory_percent)}</dd>
                  </div>
                  <div className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                    <dt className="text-[11px] text-slate-500">Health</dt>
                    <dd className="mt-0.5">
                      <HealthBadge score={selected.health_score} grade={selected.health_grade} />
                    </dd>
                  </div>
              ...
              <section>
                ...
                <DeviceMetricsChart deviceId={String(selected.id)} />
              </section>

              <section className="flex flex-wrap gap-2">
                <Link
                  href={`/remote?agent=${encodeURIComponent(String(selected.id))}&mode=desktop`}
                  className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-sm font-semibold text-white hover:bg-indigo-500"
                >
                  <ScreenShare size={16} /> Remote desktop
                </Link>
                <Link
                  href={`/remote?agent=${encodeURIComponent(String(selected.id))}&mode=terminal`}
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                >
