...
function isVersionAtLeast(actual?: string, minimum = MIN_REMOTE_DESKTOP_AGENT_VERSION): boolean | null {
  const a = parseVersion(actual); const b = parseVersion(minimum); if (!a || !b) return null;
  for (let i = 0; i < Math.max(a.length, b.length); i += 1) { const av = a[i] ?? 0; const bv = b[i] ?? 0; if (av > bv) return true; if (av < bv) return false; }
  return true;
}
function apiHttpBase(): string { const raw = (process.env.NEXT_PUBLIC_API_URL || 'https://bhudi-online-production.up.railway.app').trim(); return raw.replace(/\/$/, ''); }
function wsBase(): string { return apiHttpBase().replace(/^http/, 'ws'); }
function authHeaders(): HeadersInit { return { 'Content-Type': 'application/json', Accept: 'application/json' }; }
function mouseButtonName(button: number): 'left' | 'middle' | 'right' { if (button === 2) return 'right'; if (button === 1) return 'middle'; return 'left'; }

export default function RemoteAccessConsole() {
  const { blocked: mfaBlocked } = useMfaGate(); const searchParams = useSearchParams();
  const [devices, setDevices] = useState<Device[]>([]); const [agentId, setAgentId] = useState('');
  const [mode, setMode] = useState<'desktop' | 'terminal'>('desktop'); const [shell, setShell] = useState('powershell');
  ...
  async function startSession() {
    ...
    try {
      ...
      const res = await fetch(API_BASE + path, { method: 'POST', headers: authHeaders(), credentials: 'include', cache: 'no-store', body: JSON.stringify(body) });
  ...
  function onCanvasMouseUp(e: React.MouseEvent<HTMLCanvasElement>) { e.preventDefault(); const { x, y } = canvasCoords(e); sendInput({ type: 'mouseup', button: mouseButtonName(e.button), x, y }); }
  function onCanvasClick(e: React.MouseEvent<HTMLCanvasElement>) { e.preventDefault(); }
  function onCanvasMouseMove(e: React.MouseEvent<HTMLCanvasElement>) { const { x, y } = canvasCoords(e); sendInput({ type: 'mousemove', x, y }); }
  function onCanvasWheel(e: React.WheelEvent<HTMLCanvasElement>) { e.preventDefault(); const { x, y } = canvasCoords(e); sendInput({ type: 'wheel', x, y, deltaY: e.deltaY }); }
  function onCanvasKeyDown(e: React.KeyboardEvent<HTMLCanvasElement>) { e.preventDefault(); sendInput({ type: 'keydown', key: e.key, code: e.code }); }
  function onCanvasKeyUp(e: React.KeyboardEvent<HTMLCanvasElement>) { e.preventDefault(); sendInput({ type: 'keyup', key: e.key, code: e.code }); }

  return (<div className="space-y-4">
    {mfaBlocked && <MfaRequiredPanel title="Remote access locked" />}
    <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="min-w-[220px] flex-1"><label className="mb-1 block text-xs font-medium text-slate-500">Device / agent</label><select value={agentId} onChange={(e) => setAgentId(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" disabled={mfaBlocked}><option value="">Select device…</option>{devices.map((d) => { const id = deviceAgentId(d); const label = d.hostname || d.name || id; return <option key={id} value={id}>{label} ({d.status || 'unknown'})</option>; })}</select></div>
      <div className="flex gap-2"><button type="button" onClick={() => setMode('desktop')} disabled={mfaBlocked} className={'inline-flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-medium ' + (mode === 'desktop' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700') + (mfaBlocked ? ' opacity-50' : '')}><Monitor className="h-4 w-4" /> Desktop</button><button type="button" onClick={() => setMode('terminal')} disabled={mfaBlocked} className={'inline-flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-medium ' + (mode === 'terminal' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700') + (mfaBlocked ? ' opacity-50' : '')}><Terminal className="h-4 w-4" /> Terminal</button></div>
      {mode === 'desktop' && <div><label className="mb-1 block text-xs font-medium text-slate-500">Display</label><select value={monitorIndex} onChange={(e) => setMonitorIndex(Number(e.target.value))} className="rounded-xl border border-slate-200 px-3 py-2 text-sm" disabled={mfaBlocked}>{monitors.length === 0 ? <option value={0}>Primary / Display 1</option> : monitors.map((m) => <option key={m.index} value={m.index}>{m.name || `Display ${m.index + 1}`}{m.primary ? ' ★' : ''}</option>)}</select></div>}
