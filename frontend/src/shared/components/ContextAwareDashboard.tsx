...
function matchesScope(row: Row, organizationId: string | null, siteId: string | null, tenantId?: string) {
  ...
  const candidates = [row.organization_id, row.organizationId, row.site_id, row.siteId, row.tenant_id, row.tenantId].filter(Boolean).map(String);
  if (siteId && candidates.includes(String(siteId))) return true;
  if (organizationId && candidates.includes(String(organizationId))) return true;
  if (tenantId && candidates.includes(String(tenantId))) return true;
  // Do not pretend an unscoped record belongs to a customer.
  return false;
}
...
export default function ContextAwareDashboard() {
  ...
  useEffect(() => { if (!authLoading && !workspaceLoading && user) void load(); }, [organizationId, siteId]);
  ...
  const hasScopedRecords = useMemo(() => scopedDevices.length > 0 || scopedTickets.length > 0, [scopedDevices, scopedTickets]);
  const stats = useMemo(() => {
