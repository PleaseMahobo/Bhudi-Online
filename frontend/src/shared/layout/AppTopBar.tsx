...
import WorkspaceSelector from '@/shared/layout/WorkspaceSelector';
...
export default function AppTopBar({ title, onMenuClick }: { title?: string; onMenuClick?: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex min-h-16 shrink-0 flex-wrap items-center gap-2 border-b border-slate-200 bg-white/95 px-4 py-2 backdrop-blur md:px-6">
      ...
      <div className="hidden min-w-0 md:block md:w-44 lg:w-52"><p className="truncate text-sm font-semibold text-slate-900">{title || 'Workspace'}</p><p className="truncate text-[11px] text-slate-500">IT Operations</p></div>
      <WorkspaceSelector />
      <GlobalSearch />
      <div className="ml-auto flex items-center gap-1">
        ...
        <div className="relative ml-1">
          <button type="button" onClick={() => setProfileOpen((value) => !value)} className="flex items-center gap-2 rounded-lg p-1.5 pr-2 hover:bg-slate-100" aria-expanded={profileOpen}>
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-xs font-semibold text-white">{initials}</span><span className="hidden max-w-36 truncate text-left text-xs font-medium text-slate-700 xl:block">{user?.email || 'User'}</span><ChevronDown size={14} className="hidden text-slate-400 xl:block" />
          </button>
