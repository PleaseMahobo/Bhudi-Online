...
def test_agent_platform_exposes_agent_details_and_command_history() -> None:
    ...
    enroll_response = client.post(
        ...
        json={
            ...
            "agent_version": "1.2.3",
            "platform": "linux",
            "enrollment_secret": "phase-3-test",
        },
    )
    ...
    queue_response = client.post(
        ...
        json={"command": "echo hello", "shell": True},
    )
    ...
    result_response = client.post(
        f"/runtime/agents/{agent_id}/commands/{command_id}/result",
        params={"agent_token": agent_token},
        json={"exit_code": 0, "stdout": "hello", "stderr": ""},
    )
...
def test_agent_commands_support_acknowledgement_and_retry_tracking() -> None:
    ...
    enroll_response = client.post(
        "/runtime/enroll",
        json={"hostname": "agent-02", "agent_version": "1.0.0", "platform": "windows"},
    )
    body = enroll_response.json()
    agent_id = body["agent_id"]
    agent_token = body["agent_token"]
    ...
    queue_response = client.post(
        f"/runtime/agents/{agent_id}/commands",
        json={"command": "python -c 'exit(1)'", "shell": True},
    )
    command_id = queue_response.json()["command_id"]

    ack_response = client.post(
        f"/runtime/agents/{agent_id}/commands/{command_id}/ack",
        params={"agent_token": agent_token},
        json={"status": "running"},
    )
    ...
    result_response = client.post(
        f"/runtime/agents/{agent_id}/commands/{command_id}/result",
        params={"agent_token": agent_token},
        json={"exit_code": 1, "stdout": "", "stderr": "boom"},
    )
    assert result_response.status_code == 200

    history_response = client.get(f"/runtime/agents/{agent_id}/commands")
    command = history_response.json()["commands"][0]
...
def test_agent_stream_endpoint_accepts_connections() -> None:
    ...
    enroll_response = client.post(
        "/runtime/enroll",
        json={"hostname": "agent-03", "agent_version": "1.0.0", "platform": "linux"},
    )
    agent_id = enroll_response.json()["agent_id"]
