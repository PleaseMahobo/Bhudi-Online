...
async def secure_enroll(req: EnrollRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if existing is not None:
        ...
        _agents[agent_id] = {"agent_id": agent_id, "agent_token": agent_token, "tenant_id": str(enrollment.tenant_id), "hostname": req.hostname, "agent_version": req.agent_version, "platform": req.platform, "platform_metadata": _platform_metadata(req.platform), "status": "online", "registered_at": existing.registered_at.isoformat(), "last_seen": now.isoformat(), "cpu_percent": None, "memory_percent": None, "disk_percent": None, "ip_address": None, "enrollment_secret": None, "commands_completed": 0, "commands_failed": 0}
    ...
    _agents[agent_id] = {"agent_id": agent_id, "agent_token": agent_token, "tenant_id": str(enrollment.tenant_id), "hostname": req.hostname, "agent_version": req.agent_version, "platform": req.platform, "platform_metadata": _platform_metadata(req.platform), "status": "online", "registered_at": now.isoformat(), "last_seen": now.isoformat(), "cpu_percent": None, "memory_percent": None, "disk_percent": None, "ip_address": None, "enrollment_secret": None, "commands_completed": 0, "commands_failed": 0}
    _commands[agent_id] = []

    row = Agent(id=uuid.UUID(agent_id), hostname=req.hostname, agent_version=req.agent_version, platform=req.platform, machine_guid=machine_guid, tenant_id=enrollment.tenant_id, enrollment_token=agent_token, registration_state="approved", approved=True, trusted=True, status="online", enabled=True, registered_at=now, last_seen=now, last_heartbeat=now)
    db.add(row)
    db.flush()
