...
def _normalize_row(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        health_score = None
    grade = raw.get("health_grade")
    if grade is None and health_score is not None:
        if health_score >= 90:
            grade = "A"
    ...
    return {
        ...
        "health_grade": grade,
        "consecutive_misses": raw.get("consecutive_misses"),
                ...
                row = _normalize_row(
                    {
                        ...
                        "cpu_percent": getattr(d, "cpu", None),
                        "memory_percent": getattr(d, "ram", None),
                        "disk_percent": getattr(d, "disk", None),
                        "health_score": getattr(d, "health_score", None),
                        "consecutive_misses": getattr(d, "consecutive_misses", None),
                        "source": "db",
...
def request_inventory(
    ...
    try:
        ...
        result = agent_runtime.create_command(agent_id, body)
        return {
            "accepted": True,
            "command_id": result.get("command_id") or result.get("id"),
            "kind": kind,
            "command": cmd,
            "agent_id": agent_id,
        }
...
def get_device_command(
    ...
    try:
        ...
        for c in agent_runtime._commands.get(agent_id, []):
            if c.get("command_id") == command_id or c.get("id") == command_id:
                return {
