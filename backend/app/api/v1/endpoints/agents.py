...
def enroll_agent(request: AgentEnrollRequest, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
...
def authenticate_agent(request: AgentAuthenticationRequest, db: Session = Depends(get_db)):
    return AgentService(db).authenticate(agent_uuid=request.agent_uuid, enrollment_secret=request.enrollment_secret)


@router.post("/{agent_id}/approve", response_model=AgentResponse)
def approve_agent(agent_id: UUID, request: AgentApprovalRequest, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
...
def heartbeat(request: AgentHeartbeatRequest, db: Session = Depends(get_db)):
    agent = AgentService(db).heartbeat(agent_uuid=request.agent_uuid, ip_address=request.ip_address, username=request.username)
...
def publish_update(agent_id: UUID, request: AgentUpdateRequest, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
...
def queue_command(agent_id: UUID, request: AgentCommandRequest, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
    AgentService(db)._scoped_agent(agent_id, _tenant_id(user))
    return AgentCommandResponse(accepted=True, command_id=UUID("00000000-0000-0000-0000-000000000001"))


@router.post("/{agent_id}/quarantine", response_model=AgentResponse)
def quarantine(agent_id: UUID, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
    return AgentService(db).quarantine(agent_id, _tenant_id(user))


@router.post("/{agent_id}/restore", response_model=AgentResponse)
def restore(agent_id: UUID, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
    return AgentService(db).restore(agent_id, _tenant_id(user))


@router.post("/{agent_id}/revoke", response_model=AgentResponse)
def revoke(agent_id: UUID, request: AgentRevocationRequest, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
    return AgentService(db).revoke(agent_id=agent_id, reason=request.reason, tenant_id=_tenant_id(user))


@router.get("/{agent_id}", response_model=AgentResponse)
def get_agent(agent_id: UUID, db: Session = Depends(get_db), user=Depends(current_tenant_user)):
    return AgentService(db)._scoped_agent(agent_id, _tenant_id(user))


@router.get("/pending/list", response_model=list[AgentResponse])
def pending_agents(db: Session = Depends(get_db), user=Depends(current_tenant_user)):
...
def online_agents(db: Session = Depends(get_db), user=Depends(current_tenant_user)):
...
def offline_agents(db: Session = Depends(get_db), user=Depends(current_tenant_user)):
