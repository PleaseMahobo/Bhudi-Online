...
class RemediationService:
    """Production alert → remediation executor with cooldown, severity gates, and dry-run."""
        ...
        *,
        ...
        if not action.get("enabled", True):
            return self._record_skip(
                alert=alert, rule_id=rule_id, rule_name=rule_name,
                action_type=action_type or "unknown", action_name=action_name,
        ...
        if cooldown > 0 and self._in_cooldown(
            fingerprint=alert.fingerprint, action_name=action_name, cooldown_seconds=cooldown
        ):
            return self._record_skip(
                alert=alert, rule_id=rule_id, rule_name=rule_name,
                action_type=action_type, action_name=action_name,
        ...
        if dry_run or action_type == "notify_only":
            run = RemediationRun(
                alert_id=str(alert.id), rule_id=rule_id, rule_name=rule_name,
                fingerprint=alert.fingerprint, correlation_key=alert.correlation_key,
                device_id=device_id, action_type=action_type, action_name=action_name,
                command_type=action.get("command_type"),
                status="dry_run" if dry_run else "skipped",
                skip_reason=None if dry_run else "notify_only",
                dry_run=dry_run, severity=alert.severity,
                details={"action": {k: v for k, v in action.items() if k != "script_content"},
        ...
        try:
            task_id, command_type = self._queue_work(action=action, device_id=device_id, alert=alert)
            run = RemediationRun(
                alert_id=str(alert.id), rule_id=rule_id, rule_name=rule_name,
                fingerprint=alert.fingerprint, correlation_key=alert.correlation_key,
                device_id=device_id, action_type=action_type, action_name=action_name,
                command_type=command_type, status="queued", dry_run=False, task_id=task_id,
                severity=alert.severity,
                details={"action_name": action_name, "shell": action.get("shell"),
            ...
            run = RemediationRun(
                ...
                fingerprint=alert.fingerprint, correlation_key=alert.correlation_key,
                device_id=device_id, action_type=action_type, action_name=action_name,
                status="failed", skip_reason="queue_error", severity=alert.severity,
    ...
    def _queue_work(
        self, *, action: dict[str, Any], device_id: str, alert: MonitoringAlert
        ...
        self.db.add(
            Action(
                ...
                payload={"script_id": str(script.id), "task_id": str(task.id), "alert_id": str(alert.id)},
                status="queued",
                result="queued",
            )
        )
        self.db.add(AutomationLog(action="alert.remediation", result="queued"))
        self.db.flush()
        return str(task.id), command_type

    def _resolve_device_id(
        self, *, check: MonitoringCheck | None, alert: MonitoringAlert
    ) -> str | None:
        ...
        q = self.db.query(Device).filter(Device.hostname == target).first()
        if q is not None:
            return str(q.id)

        try:
            q = self.db.query(Device).filter(Device.hostname.ilike(target)).first()
            if q is not None:
                return str(q.id)
        except Exception:
            pass
    ...
    def _in_cooldown(
        self, *, fingerprint: str | None, action_name: str, cooldown_seconds: int
    ) -> bool:
        if not fingerprint:
            return False
