...
func main() {
    ...
    case "install":
        ...
        if err := installService(cfg.Server); err != nil { logInstall("install FAILED: %v", err); fatal(err) }
        logInstall("install OK")
    case "upgrade":
        cfg := parseRunFlags(os.Args[2:])
        logInstall("upgrade start version=%s server=%s", agentVersion, cfg.Server)
        if err := upgradeService(cfg.Server); err != nil { logInstall("upgrade FAILED: %v", err); fatal(err) }
        logInstall("upgrade OK")
    case "uninstall":
        logInstall("uninstall start")
        if err := uninstallService(); err != nil { logInstall("uninstall FAILED: %v", err); fatal(err) }
...
Commands:
  ...
  desktop-worker  [-server URL]   Internal interactive-session desktop capture worker
  version
