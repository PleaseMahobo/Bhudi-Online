...
func launchDesktopWorker(serverURL, sessionID, sessionMode, displayProtocol string, monitorIndex int) error {
    ...
    if err := windows.WTSQueryUserToken(sid, &token); err != nil {
        return fmt.Errorf("query user token for session %d: %w", sid, err)
    }
    defer token.Close()
    ...
    if err := windows.CreateProcessAsUser(token, exe, cmdline, nil, nil, false, flags, nil, nil, &si, &pi); err != nil {
